...
public class CartInfoServiceImpl implements CartInfoService {
    ...
    public void addToCart(Long userId, Long skuId, Integer skuNum) {
        ...
        if(Boolean.TRUE.equals(hashOperations.hasKey(skuId.toString()))) {
            ...
            Integer finalSkuNum = skuNum;
            productReactorClient.getSkuInfo(skuId).mapNotNull(skuInfo->{
                //封装cartInfo对象
                ...
                cartInfo.setSkuNum(finalSkuNum);
                cartInfo.setCurrentBuyNum(finalSkuNum);
                ...
                this.setCartKeyExpire(cartKey);
                return null;
            }).switchIfEmpty(Mono.error(new SsyxException(ResultCodeEnum.DATA_ERROR))).subscribeOn(Schedulers.parallel()).subscribe();
        }
    }
    ...
    //购物车列表
    @Override
    public Mono<List<CartInfo>> getCartList(Long userId) {
        //判断userId
        List<CartInfo> cartInfoList = new ArrayList<>();
        if(ObjectUtils.isEmpty(userId)) {
            return Mono.just(cartInfoList);
        }
        //从redis获取购物车数据
        String cartKey = this.getCartKey(userId);
    ...
    public Mono<List<CartInfo>> getCartCheckedList(Long userId) {
        ...
        //isChecked = 1购物项选中
        List<CartInfo> cartInfoListNew = cartInfoList
                .stream()
                .filter(cartInfo -> cartInfo.getIsChecked() == 1)
                .collect(Collectors.toList());
        return Mono.just(cartInfoListNew);
    }
    ...
    public void deleteCartChecked(Long userId) {
        //根据userid查询选中购物车记录
        getCartCheckedList(userId).mapNotNull(cartInfoList->{
